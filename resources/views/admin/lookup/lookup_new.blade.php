...
<main class="col-md-9 ms-sm-auto col-lg-10 main-content">
   ...
   <div class="container-fluid">
      <form id='lookup-form' method="post" action="{{ url('lookup-add') }}">
       @csrf
         <div class="row">
           <div class="col-md-4">            
            <label for="lookup_group" class="form-label">Lookup Group</label><span class="text-danger"> *</span>
            <input type="text" class="form-control" name="lookup_group" value="{{ old('lookup_group') }}">
            @error('lookup_group')
          ...
          <div class="col-md-4">
            ...
            <input type="text" class="form-control" name="lookup_label" value="{{old('lookup_label')}}">
            @error('lookup_label')
            <p class="text-danger">{{ $message }}</p>
            @enderror
         </div>
        ...
        <div class="row">
          <div class="col-md-4">
            ...
            <input type="text" class="form-control" name="lookup_value" value="{{old('lookup_value')}}">
            @error('lookup_value')
            <p class="text-danger">{{ $message }}</p>
            @enderror
          </div>
          <div class="col-md-4">
            ...
            <input type="text" class="form-control" name="lookup_order" value="{{old('lookup_order')}}">
            @error('lookup_order')
            <p class="text-danger">{{ $message }}</p>
            @enderror
          </div>
